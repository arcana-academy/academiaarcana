...
async function remove(storeName: string, id: string): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    transaction.oncomplete = () => resolve();
    transaction.onerror = () => reject(transaction.error ?? new Error("Falha ao remover o item local."));
  });
  db.close();
}
...
async function getAll<T>(storeName: string): Promise<T[]> {
  ...
  return new Promise<T[]>((resolve, reject) => {
    ...
    request.onerror = () => { db.close(); reject(request.error ?? new Error("Falha ao ler o armazenamento local.")); };
  });
}

export const localSyncStore = {
  saveVersion(version: DocumentVersion) { return put(VERSIONS, version); },
  listVersions() { return getAll<DocumentVersion>(VERSIONS); },
  enqueue(operation: SyncOperation) { return put(QUEUE, operation); },
  acknowledge(operationId: string) { return remove(QUEUE, operationId); },
  listPendingOperations() { return getAll<SyncOperation>(QUEUE); },
};
